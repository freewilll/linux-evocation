...
/* Give the socket FD the local address ADDR (which is LEN bytes
   long).  */
int bind __P ((int __sockfd, struct sockaddr *__my_addr,
		int __addrlen));
...
int connect __P ((int __sockfd, struct sockaddr *__serv_addr,
		int __addrlen));
...
int accept __P ((int __sockfd, struct sockaddr *__peer,
		int *__paddrlen));
...
int send __P ((int __sockfd, __const void *__buff, int __len,
		 unsigned int __flags));

/* Read N bytes into BUF from socket FD.
   Returns the number read or -1 for errors.  */
int recv __P ((int __sockfd, void *__buff, int __len,
		 unsigned int __flags));

/* Send N bytes of BUF on socket FD to peer at address ADDR (which is
   ADDR_LEN bytes long).  Returns the number sent, or -1 for errors. */
int sendto __P ((int __sockfd, __const void *__buff, int __len,
...
int recvfrom __P ((int __sockfd, void *__buff, int __len,
